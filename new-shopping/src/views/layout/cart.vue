    ...
    <div class="cart-box" v-if="isLogin && cartList.length > 0">
      ...
      <div class="cart-list">
        <div class="cart-item" v-for="item in cartList" :key="item.goods_id">
          <van-checkbox icon-size="18" @click="toggleCheck1(item.goods_id)" :value="item.isChecked"></van-checkbox>
      ...
      <div class=" footer-fixed">
        ...
        <div class="all-total">
          ...
          <div v-if="!isEdit" :class="{ disabled: selCount === 0 }" @click="goPay" class="goPay">
            结算({{ selCount }})
          </div>
...
export default {
  ...
  data () {
    return {
      isEdit: false,
      sss: this.$store.state.cart.cartList
    }
  },
  ...
  created () {
    // const token = this.$store.getters['cart/getToken']
    ...
    goPay () {
      if (this.selCount > 0) {
        ...
        this.$router.push({
          ...
          query: {
            mode: 'cart',
            cartIds: this.cartList.map(item => item.goods_id).join(',')
